...
const FORECAST_ENDPOINT = `https://delois-sandlike-widely.ngrok-free.dev/api/weather/previsao?lat=${address.lat}&lon=${address.lon}&day=${date.day}&month=${date.month}&year=${date.year}`;
...
const TemperatureChart = ({ data }) => {
  ...
  return (
    <div className="mb-5 rounded-lg bg-[#1e2746] p-4 text-white w-[330px] no-scrollbar">
      ...
      <div className="h-[220px] overflow-x-auto no-scrollbar">
        {chartRows.length ? (
          <ResponsiveContainer width={ chartRows.length * 60} height="100%">
            <ComposedChart
              data={chartRows}
      ...
      <div className="mt-3 flex flex-no-wrap gap-4 text-sm overflow-x-auto no-scrollbar">
